...
- First vertical slice：Implemented as a technical slice; blocking Region Selection UX correction required.
- Blocking defect：The current selection surface can obscure the desktop and the implementation can acquire the final source frame after selection. The required flow is one frozen source frame before selection、visible selection on that frame、and final crop from that exact frame.
- Feature expansion：Paused until the corrected Capture／Selection workflow is verified.
