...
- 正常 Capture Workflow 與一般自動化測試不得啟動 Paint 或其他外部程式；會啟動外部 GUI fixture 的 Interactive Verification 必須先取得明確授權。
...
| `SPEC-0005-AC-019` | 一般自動化測試不得啟動外部 GUI；任何需要外部 GUI fixture 的 Interactive Verification 都必須先取得明確授權。 | `NFR-004`、`NFR-006` |
