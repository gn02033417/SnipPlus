...
The Clipboard adapter accepts one immutable final rendered image without owning Capture、Selection、Annotation、PNG Output or shared workflow state. It also does not own deletion or rollback of a PNG already created by the Save feature.
...
1. Encode the immutable final result as an in-memory PNG stream with alpha preserved.
...
6. Execute publication on the appropriate WinUI／Windows apartment.
7. Call `Clipboard.Flush()` after successful publication.
...
12. The adapter never mutates shared workflow state、declares the SnipPlus session complete or deletes a saved PNG.
...
- the Clipboard failure is disclosed;
- the current Editing state is preserved;
- the PNG remains at the user-selected destination;
- the Clipboard adapter returns the failure and any supplied File Reference without deleting or rolling back the file.
...
- A Save-originated request may also carry a retained File Reference for workflow feedback.
...
- User-created PNG files are owned by the Output result and are not temporary Clipboard resources.
...
Failure handling does not overwrite the user’s previous Clipboard content unless replacement actually succeeded. A Save-originated failure does not delete the retained PNG.

## Verification Requirements

- Successful publication and paste into representative Windows consumers.
- Alpha behavior、including transparent Virtual Desktop gap pixels.
...
- Clipboard failure after PNG success retains the file and reports a partial outcome.
...
- Save integration、retained-file outcome and transparent cross-display output are not implemented.
- The adapter remains a reusable technical foundation behind explicit Complete and Save commitments.
