...
狀態：`Reviewed — resident lifecycle, PrintScreen request boundary, Windows multi-display freezing integration and Owner Reference packaged Frozen Display Presentation／Crosshair／initial Selection runtime verified; Maximum four-4K envelope and later Selection／Editing／output slices remain partial or missing`
...
| `FR-006`–`FR-010` | Freeze and present all supported displays、crosshair、cross-monitor Selection and mask. | Current Owner Reference topology、physical bounds／DPI、capacity and real per-display WGC complete frame-set acquisition are verified. The fifth slice presents one physical overlay per display with its frozen frame, dim-outside／clear-inside mask、crosshair and initial cross-monitor selection; current packaged runtime verified the Owner Reference three-display path and no partial 1/2-overlay startup state. Maximum four-4K runtime remains unverified. | `Partial` | Preserve Owner Reference conformance and separately verify the broader four-4K envelope. |
...
| `FR-036`–`FR-038` | Esc cancels at accepted capture stages. | Platform-neutral Esc routing is statically implemented for `Freezing`、`Selecting` and `SelectionLocked`; deterministic tests and current packaged runtime cover Drag-before、Drag-middle、Lower-display-focus and SelectionLocked overlay cleanup. Later accepted stages remain unimplemented. | `Partial` | Preserve packaged overlay cancellation and verify later accepted stages. |
...
| `FR-040`–`FR-043` | Cleanup、focus restoration、no MainWindow reopening、window exclusion. | Source exclusion／hide is statically performed before frame acquisition; current packaged runtime verified MainWindow exclusion、overlay/session cleanup and no automatic MainWindow reopening. Focus restoration remains unverified. | `Partial` | Add foreground-context behavior and preserve packaged cleanup. |
...
| `FrozenDisplayOverlayPlanBuilder`／`InitialSelectionCoordinator`／`CapturePresentationWorkflowCoordinator` | `Conforms` for Owner Reference packaged Overlay／Crosshair／initial Selection runtime | Builds one physical overlay descriptor per display, presents only complete frame sets, maps pointer input to normalized physical Virtual Desktop bounds, and routes valid release／Esc through COMP-001 with deterministic cleanup tests. Current packaged runtime verified complete three-overlay presentation, five single／cross-monitor drag paths, SelectionLocked and no-output cancellation; later adjustment remains pending. | Preserve single-session and single-frame-set ownership; keep later adjustment separate. |
| `WindowsFrozenDisplayOverlayCoordinator`／`WindowsMainWindowCaptureSourceExclusion` | `Conforms` for Owner Reference packaged Overlay／Crosshair／initial Selection runtime | Provides one per-display physical overlay, PMA V2 placement, own frozen frame presentation, dim-outside／clear-inside mask, and shared pointer capture; Crosshair is visible only on the display containing the current Pointer, preventing inactive-display ghosts. MainWindow is hidden before acquisition and closes idempotently. Runtime fixes keep activation hidden and batch the final show, producing only `0 → 3` display-sized overlay visibility. | Preserve the verified Owner Reference behavior; keep Maximum four-4K and topology-change runtime separate. |
...
4. All-display presentation、crosshair and cross-monitor initial Selection — fifth slice static implementation、deterministic verification and Owner Reference packaged runtime verification complete; broader Maximum four-4K capacity remains pending.
