...
狀態：`Reviewed — resident lifecycle, PrintScreen request boundary and Windows multi-display freezing integration retained; Frozen Display Presentation and initial Selection are statically implemented with deterministic evidence; real Overlay／Selection runtime remains pending`
...
| `FR-006`–`FR-010` | Freeze and present all supported displays、crosshair、cross-monitor Selection and mask. | Current Owner Reference topology、physical bounds／DPI、capacity and real per-display WGC complete frame-set acquisition are verified; the fifth slice statically adds one physical overlay per display, own frozen-frame presentation, dim-outside／clear-inside mask、crosshair and initial cross-monitor selection geometry with deterministic evidence. Real packaged Overlay／Crosshair／Selection runtime is not yet executed. | `Partial` | Execute the separately authorized packaged presentation／Selection runtime; preserve per-display frame ownership. |
...
| `FrozenDisplayOverlayPlanBuilder`／`InitialSelectionCoordinator`／`CapturePresentationWorkflowCoordinator` | `Partial` fifth-slice static implementation | Builds one physical overlay descriptor per display, presents only complete frame sets, maps pointer input to normalized physical Virtual Desktop bounds, and routes valid release／Esc through COMP-001 with deterministic cleanup tests. Real packaged Overlay／Crosshair／Selection runtime and later adjustment remain pending. | Execute the separately authorized runtime verification; preserve single-session and single-frame-set ownership. |
| `WindowsFrozenDisplayOverlayCoordinator`／`WindowsMainWindowCaptureSourceExclusion` | `Partial` fifth-slice Windows integration | Provides one per-display physical overlay, PMA V2 placement, own frozen frame presentation, dim-outside／clear-inside mask, crosshair and shared pointer capture; hides MainWindow before acquisition and closes idempotently. No real Windows overlay runtime has been executed for this slice. | Verify packaged presentation／selection and focus behavior on the Owner Reference topology. |
...
4. All-display presentation、crosshair and cross-monitor initial Selection — fifth slice static implementation and deterministic verification complete; packaged runtime pending.
