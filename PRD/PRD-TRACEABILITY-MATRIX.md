...
狀態：`Stage 5 Repository owner manual acceptance passed for the Owner Reference environment with the documented Windows PrintScreen compatibility prerequisite; Stage 6A／6B implementation and deterministic evidence include the cross-display release、Cancel dispatch and Function Bar contrast corrections, while post-fix packaged Stage 6 runtime re-acceptance remains pending; the current packaged launch attempt failed before MainWindow with a Windows UI runtime error; later workflow stages remain unstarted or unverified`
...
- packaged Windows runtime verification of the Stage 6B Editing／Function Bar foundation;
...
| `FR-013`–`FR-016` | Mandatory Editing function bar and Complete／Save／Cancel. | Stage 6B adds the platform-neutral `SelectionLocked → Editing → Cancelled` state boundary, physical Work Area／DPI placement with Below／Above／clamp policy, one hosted Function Bar per capture session and stale session／revision cleanup. The initial Selection release failure was traced to measuring a `Visibility.Collapsed` root; preparation now measures a layout-participating root hidden by `Opacity=0` and `IsHitTestVisible=false`, then shows it only after `Editing`. The owner-reported Cancel risk is addressed by a post-routed-event DispatcherQueue dispatch and single-command gate; shared button styling provides white text on a dark background. Complete、Save、Undo and Redo remain disabled; Cancel is the only enabled command. Deterministic Core／Windows tests pass; Repository owner packaged re-acceptance remains pending. | `Partial` | Re-run packaged Stage 6A／6B runtime; implement Complete／Save in later slices. |
...
| `NFR-014`–`NFR-019` | Familiar interaction、silent success、progress and retained-error feedback. | All-display dim-outside／clear-inside mask、crosshair and initial drag geometry are statically implemented. Stage 6B adds the hosted Editing Function Bar and typed Cancel boundary; the initial release measurement lifecycle is corrected, Cancel dispatch is deferred outside the routed-event stack, and shared button styling provides deterministic high-contrast foreground/background evidence. Progress、output、retained-error and visible-success behavior remain incomplete. | `Partial／Incorrect` | Verify Stage 6B presentation and cleanup in packaged runtime, then implement later progress and output feedback. |
...
| `NFR-032`–`NFR-036` | Maintainability and traceability. | Canonical documents、single `WorkflowStateAuthority`、platform-neutral request／freezing／presentation／selection／Function Bar boundaries and deterministic capacity／session／ownership／geometry／placement tests exist. Stage 6A and Stage 6B add typed contracts without a second workflow authority; output state graph remains unimplemented. | `Conforms／Partial` | Trace every corrected slice and preserve COMP-001 ownership. |
...
| Current `WorkflowState` graph | `Partial` | Formal `ResidentReady → CaptureRequested → Freezing → Selecting → SelectionLocked → Editing → Cancelled` transitions are now legal through the single authority with deterministic tests; output states remain outside Stage 6B. |
...
| `FrozenDisplayOverlayPlanBuilder`／`InitialSelectionCoordinator`／`CapturePresentationWorkflowCoordinator` | `Partial` for Stage 5 Owner Reference plus Stage 6A／6B static slices | Builds one physical overlay descriptor per display, presents only complete frame sets, maps pointer input to normalized physical Virtual Desktop bounds, and routes valid release／Esc through COMP-001. Stage 6A adds locked move／resize／reselection with revision and rollback; Stage 6B adds `SelectionLocked → Editing`, stale Function Bar command boundaries, adjustment hide／reposition／show and Cancel cleanup while preserving one authority. The Stage 6 correction adds deterministic cross-Overlay release deduplication, Cancel dispatch gating and Virtual Desktop boundary evidence; packaged runtime remains pending. | Execute Stage 6A／6B runtime without changing session or workflow ownership. |
| `WindowsFrozenDisplayOverlayCoordinator`／`WindowsMainWindowCaptureSourceExclusion` | `Partial` for Stage 6A／6B static overlay presentation | Provides one per-display physical overlay, PMA V2 placement, own frozen frame presentation, dim-outside／clear-inside mask, one session-owned native/XAML release boundary, eight physical handle surfaces and hit-test cursor mapping. Stage 6B hosts one Function Bar inside the anchor overlay, keeps it measurable while hidden with opacity／hit-test controls, reads physical Work Area, applies DPI-aware Below／Above／clamp placement, defers Cancel cleanup outside the Button routed event and exposes only Cancel with high-contrast styling. Stage 5 initial Selection and Esc cleanup are owner-accepted; post-fix Stage 6A／6B runtime remains pending. | Execute Owner Reference Stage 6A／6B runtime; keep Maximum four-4K and later output work separate. |
...
7. Function Bar、Cancel and placement foundation — Stage 6B static implementation and deterministic tests passed, including high-contrast button policy and single-dispatch Cancel gate; Complete／Save、progress and focus restoration remain later work.
...
The corrected Stage 5 resident lifecycle／PrintScreen／activation source and fifth-slice Owner Reference Overlay／Crosshair／initial Selection acceptance remain recorded with the documented Windows PrintScreen compatibility prerequisite. Stage 6A adds platform-neutral SelectionLocked adjustment and Stage 6B adds the Editing／Function Bar／Cancel foundation while preserving `COMP-001` as the sole workflow authority. The current Stage 6 correction has locked restore、Release x64 build、132 deterministic non-interactive tests、limited C# format verification and `git diff --check` evidence. The clean-commit Development MSIX was signed、installed and hash-matched to the Release／package／installed DLL, but its launch attempt failed before MainWindow appeared with WER `Microsoft.UI.Xaml.dll` exception `0xc000027b`; therefore no post-fix packaged Stage 6A／6B runtime behavior is claimed and no source change was made for the unlocalized startup/runtime blocker. Complete／Save、Annotation、Clipboard、PNG、transparent gap output、Maximum four-4K runtime and performance evidence are not claimed. Stage 7 and later slices have not started.
