...
狀態：`Implementation and automated verification passed; Repository owner Stage 5 re-acceptance pending — resident lifecycle、single-instance activation、application-owned PrintScreen boundary and fifth-slice Frozen Display Presentation／initial Selection foundations are implemented; current packaged background hotkey／Esc／activation behavior and Maximum four-4K envelope remain unverified or partial`
...
| `FR-002` | User controls PrintScreen takeover. | `IPrintScreenTakeover`、persisted settings store and Windows `RegisterHotKey`／`UnregisterHotKey` implementation remain in place; deterministic tests cover settings、idempotent registration／release and failure state. The current source now uses an application-owned message-only HWND, but current packaged registration／restart behavior has not been re-verified after that owner change. | `Partial` | Re-run packaged setting and registration verification without starting Stage 6. |
...
| `FR-004` | Disabled takeover or application exit does not intercept PrintScreen. | Resident lifecycle release and late-event boundaries are deterministic-tested. The current message-only owner and current packaged background／post-exit behavior are pending re-verification. | `Partial` | Re-run packaged disable／exit and background PrintScreen verification. |
...
| `FR-036`–`FR-038` | Esc cancels at accepted capture stages. | Platform-neutral Esc routing is statically implemented for `Freezing`、`Selecting` and `SelectionLocked`; deterministic cleanup tests pass. The current overlay batch creates one session Escape focus owner after atomic show, while the owner’s reported pre-drag Esc failure and the previous process-termination correction remain pending current packaged re-acceptance. Later accepted stages remain unimplemented. | `Partial` | Re-run current packaged pre-drag、drag and SelectionLocked Esc scenarios; verify later accepted stages separately. |
...
| `FR-040`–`FR-043` | Cleanup、focus restoration、no MainWindow reopening、window exclusion. | Source exclusion／hide is statically performed before frame acquisition; deterministic overlay/session cleanup and no-output tests pass. Current activation routing shows the existing MainWindow only in `ResidentReady` and ignores active capture／exit activation. Current packaged Esc cleanup、resident process retention、no MainWindow reopening and focus behavior remain unverified. | `Partial` | Re-run current packaged five-item Stage 5 acceptance and add later focus restoration behavior separately. |
...
| `NFR-020`–`NFR-023` | Focus and exit behavior. | Direct MainWindow X exit、takeover release、custom AppInstance activation routing and resident activation boundary are statically implemented; deterministic lifecycle／activation tests pass. Current packaged pre-drag Esc、background PrintScreen、MainWindow reactivation and process cleanup remain unverified; full focus restoration belongs to a later slice. | `Partial` | Re-run current packaged Stage 5 acceptance and add focus restoration later. |
...
| `ResidentLifecycleCoordinator` and PrintScreen contracts | `Partial` for the corrected resident lifecycle／takeover slice pending owner re-acceptance | Added setting、registration、event-boundary and exit cleanup ownership; deterministic non-interactive evidence passes. Current source now separates the message-only PrintScreen owner from MainWindow, so the current packaged background registration／release evidence is pending. |
| `WindowsPrintScreenTakeover` | `Partial` for the corrected resident lifecycle／takeover slice pending owner re-acceptance | Added Win32 registration／release and an application-owned message-only HWND boundary; deterministic adapter tests pass. Current packaged background `WM_HOTKEY` delivery and native PrintScreen suppression are not yet verified. |
...
| `WindowsFrozenDisplayOverlayCoordinator`／`WindowsMainWindowCaptureSourceExclusion` | `Partial` for fifth-slice automated implementation; Repository owner Stage 5 re-acceptance pending | Provides one per-display physical overlay, PMA V2 placement, own frozen frame presentation, dim-outside／clear-inside mask, shared pointer capture and a system `InputSystemCursorShape.Cross` on each input surface. The former per-overlay XAML crosshair lines were removed after owner acceptance found simultaneous static crosses. Esc now has one session focus owner after atomic show and still queues the workflow boundary after the `KeyDown` callback returns, avoiding synchronous close of the focused overlay; deterministic cleanup tests pass. The new current package has not yet been run. | Complete current packaged background PrintScreen、pre-drag Esc、reactivation and MainWindow X re-acceptance; keep Maximum four-4K and topology-change runtime separate. |
...
The corrected Stage 5 resident lifecycle／PrintScreen／activation source now has locked restore、Release x64 build with `0 warnings / 0 errors`、limited format verification and `101/101` deterministic non-interactive tests passing. The implementation uses a custom pre-XAML `AppInstance` single-instance entry point, an application-owned message-only PrintScreen owner and one post-batch session Escape focus owner. Current packaged background `WM_HOTKEY` delivery、native PrintScreen suppression、pre-drag Esc、repeated Start-menu activation、MainWindow reactivation、process retention／exit and the five-item Repository owner re-acceptance have not yet been run, so Stage 5 remains `Partial／re-acceptance pending`. The second request-boundary slice has static implementation and prior packaged runtime evidence: PrintScreen and the secondary in-app command enter the sole `COMP-001` authority at `ResidentReady → CaptureRequested`. The third and fourth slices provide the platform-neutral four-4K capacity、Frozen Virtual Desktop、Capture Session、per-display frame ownership、Windows topology mapping and per-display WGC freezing integration; current Owner Reference three-display physical topology and real per-display WGC freezing passed. Maximum four-4K runtime、topology-change runtime、Selection adjustment、Editing、output and Annotation work are not claimed. No sixth slice is started.
