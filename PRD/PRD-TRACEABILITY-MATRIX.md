...
| `FR-030` | Complete renders current revision and writes Clipboard only. | Complete validates the current session／coordinate version／Selection revision and locked geometry, renders from the same `FrozenDisplayFrameSet`, creates a canonical BGRA8／premultiplied／sRGB SDR result, delivers only through `IClipboardDeliveryService` and closes to `ResidentReady` on success. History／roaming remain disabled; deterministic synthetic tests cover no recapture and transparent gaps. `CompleteExecutionStage` records the render／result／Clipboard boundaries without pixels or payloads. The packaged trace showed render／PNG success followed by `CO_E_NOTINITIALIZED` during Clipboard publication; the adapter dispatches the exact synchronous `SetContentWithOptions()`／`Flush()` calls and now records thread／dispatcher／call-boundary diagnostics. Post-fix packaged Complete／paste acceptance remains unobserved. | `Partial` | Re-run packaged Complete once and use the expanded JSONL trace plus actual paste to identify the remaining failure; retain this boundary for later Annotation／Save work. |
