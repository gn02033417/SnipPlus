...
| `FR-030` | Complete renders current revision and writes Clipboard only. | Complete validates the current session／coordinate version／Selection revision and locked geometry, renders from the same `FrozenDisplayFrameSet`, creates a canonical BGRA8／premultiplied／sRGB SDR result, delivers only through `IClipboardDeliveryService` and closes to `ResidentReady` on success. History／roaming remain disabled; deterministic synthetic tests cover no recapture and transparent gaps. `CompleteExecutionStage` records the render／result／Clipboard boundaries without pixels or payloads. The packaged trace showed render／PNG success followed by `CO_E_NOTINITIALIZED` during Clipboard publication; the adapter now dispatches the exact synchronous `SetContentWithOptions()`／`Flush()` calls, not only the surrounding async delivery. Post-fix packaged Complete／paste acceptance remains unobserved. | `Partial` | Re-run packaged Complete once and verify the local JSONL trace plus actual paste; retain this boundary for later Annotation／Save work. |
