...
狀態：`Reviewed — resident lifecycle, PrintScreen request boundary and Windows multi-display freezing integration retained; Frozen Display Presentation and initial Selection are statically implemented with deterministic evidence; packaged Overlay／Selection runtime attempted and blocked by WGC access denial`
...
| `FR-006`–`FR-010` | Freeze and present all supported displays、crosshair、cross-monitor Selection and mask. | Current Owner Reference topology、physical bounds／DPI、capacity and real per-display WGC complete frame-set acquisition are verified; the fifth slice statically adds one physical overlay per display, own frozen-frame presentation, dim-outside／clear-inside mask、crosshair and initial cross-monitor selection geometry with deterministic evidence. Current packaged verification reached WGC access preflight and returned `DeniedByUser`; no Overlay／Crosshair／Selection runtime evidence exists. | `Partial` | Resolve the Windows capture access state, then repeat the separately authorized packaged presentation／Selection runtime. |
| `FR-011` | Mouse release locks Selection and creates no output. | Initial valid release is statically routed through `COMP-001` from `Selecting → SelectionLocked`; no crop、Clipboard、PNG or output is performed. Pointer adjustment and later Editing remain outside this slice. | `Partial` | Add later SelectionLocked adjustment and output boundaries. |
...
| `FR-036`–`FR-038` | Esc cancels at accepted capture stages. | Platform-neutral Esc routing is statically implemented for `Freezing`、`Selecting` and `SelectionLocked`; deterministic tests cover cleanup and return to `ResidentReady`. Real overlay close and Windows runtime cancellation remain unverified. | `Partial` | Verify packaged overlay cancellation and later accepted stages. |
| `FR-039` | Cancel produces no output. | Fifth-slice cancellation disposes the session／frame set without crop、Clipboard、PNG or output in deterministic tests; real runtime remains pending. | `Partial` | Preserve no-output cancellation through later stages. |
| `FR-040`–`FR-043` | Cleanup、focus restoration、no MainWindow reopening、window exclusion. | Source exclusion／hide is statically performed before frame acquisition; overlay/session/frame cleanup and no automatic MainWindow reopening are statically covered. Focus restoration and real Windows overlay cleanup remain unverified. | `Partial` | Add foreground-context behavior and execute packaged runtime verification. |
...
| `FrozenDisplayOverlayPlanBuilder`／`InitialSelectionCoordinator`／`CapturePresentationWorkflowCoordinator` | `Partial` fifth-slice static implementation | Builds one physical overlay descriptor per display, presents only complete frame sets, maps pointer input to normalized physical Virtual Desktop bounds, and routes valid release／Esc through COMP-001 with deterministic cleanup tests. Packaged verification was attempted but stopped at `DeniedByUser` before frame presentation; later adjustment remains pending. | Resolve the Windows capture access state and repeat the separately authorized runtime verification; preserve single-session and single-frame-set ownership. |
| `WindowsFrozenDisplayOverlayCoordinator`／`WindowsMainWindowCaptureSourceExclusion` | `Partial` fifth-slice Windows integration | Provides one per-display physical overlay, PMA V2 placement, own frozen frame presentation, dim-outside／clear-inside mask, crosshair and shared pointer capture; hides MainWindow before acquisition and closes idempotently. Current packaged run did not reach overlay creation because WGC access preflight returned `DeniedByUser`. | Resolve the Windows capture access state, then verify packaged presentation／selection and focus behavior on the Owner Reference topology. |
...
4. All-display presentation、crosshair and cross-monitor initial Selection — fifth slice static implementation and deterministic verification complete; packaged runtime attempted and blocked by `DeniedByUser` before WGC frame acquisition.
