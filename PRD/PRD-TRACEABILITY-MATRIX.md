...
狀態：`Stage 5 Repository owner manual acceptance passed for the Owner Reference environment with the documented Windows PrintScreen compatibility prerequisite; Stage 6A SelectionLocked adjustment and Stage 6B Editing／Function Bar foundation have static implementation and deterministic evidence, Stage 6B initial Selection release correction is ready for Repository owner re-acceptance, while packaged Stage 6A／6B runtime verification remains pending; later workflow stages remain unstarted or unverified`
...
| `FrozenDisplayOverlayPlanBuilder`／`InitialSelectionCoordinator`／`CapturePresentationWorkflowCoordinator` | `Partial` for Stage 5 Owner Reference plus Stage 6A／6B static slices | Builds one physical overlay descriptor per display, presents only complete frame sets, maps pointer input to normalized physical Virtual Desktop bounds, and routes valid release／Esc through COMP-001. Stage 6A adds locked move／resize／reselection with revision and rollback; Stage 6B adds `SelectionLocked → Editing`, stale Function Bar command boundaries, adjustment hide／reposition／show and Cancel cleanup while preserving one authority. The initial release failure path is typed and covered by deterministic tests; packaged runtime remains pending. | Execute Stage 6A／6B runtime without changing session or workflow ownership. |
| `WindowsFrozenDisplayOverlayCoordinator`／`WindowsMainWindowCaptureSourceExclusion` | `Partial` for Stage 6A／6B static overlay presentation | Provides one per-display physical overlay, PMA V2 placement, own frozen frame presentation, dim-outside／clear-inside mask, shared pointer capture, eight physical handle surfaces and hit-test cursor mapping. Stage 6B hosts one Function Bar inside the anchor overlay, keeps it measurable while hidden with opacity／hit-test controls, reads physical Work Area, applies DPI-aware Below／Above／clamp placement and exposes only Cancel. Stage 5 initial Selection and Esc cleanup are owner-accepted; Stage 6A／6B runtime remains pending. | Execute Owner Reference Stage 6A／6B runtime; keep Maximum four-4K and later output work separate. |
...
6. Accepted workflow state graph — Stage 6B `SelectionLocked → Editing → Cancelled` foundation implemented and deterministically tested; packaged verification pending.
7. Function Bar、Cancel and placement foundation — Stage 6B static implementation and deterministic tests passed; Complete／Save、progress and focus restoration remain later work.
...
The corrected Stage 5 resident lifecycle／PrintScreen／activation source and fifth-slice Owner Reference Overlay／Crosshair／initial Selection acceptance remain recorded with the documented Windows PrintScreen compatibility prerequisite. Stage 6A adds platform-neutral SelectionLocked adjustment and Stage 6B adds the Editing／Function Bar／Cancel foundation while preserving `COMP-001` as the sole workflow authority. The Stage 6B initial release measurement correction has locked restore、Release x64 build、128 deterministic non-interactive tests、limited C# format verification and `git diff --check` evidence; Repository owner re-acceptance is pending and no packaged Stage 6A／6B runtime verification has been run after this correction. Complete／Save、Annotation、Clipboard、PNG、transparent gap output、Maximum four-4K runtime and performance evidence are not claimed. Stage 7 and later slices have not started.
